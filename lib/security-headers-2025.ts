...
export function generateSecurityHeaders(config: SecurityHeadersConfig = {}): Record<string, string> {
  ...
  if (config.enableCSP !== false) {
    ...
    addDirective('connect-src', csp.connectSrc || ["'self'", 'https://api.anpip.com', 'wss://api.anpip.com', 'https://fkmhucsjybyhjrgodwcx.supabase.co', 'https://vlibyocpdguxpretjvnz.supabase.co', 'wss://*.supabase.co']);
